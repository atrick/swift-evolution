...
Let's consider adding support `Span` to `Array`. Our proposal allows `array.span` property to be declared as follows:
...
In addition to function declarations, function types must support lifetime annotations so functions that return non-`Escapable` types can be passed as closures. Function types typically omit parameter names, but they are required for any parameter referenced by the annotation.
...
It would furthermore be natural to treat the argument to `addTask` as a nonescaping closure; the scope which the closure cannot escape is not the immediate `addTask` call, but the `withTaskGroup` block as a whole:
